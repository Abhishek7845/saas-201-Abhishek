...
(function() {
  ...
  var disableDuplicateSecondaryDepartment = function(event) {
    ...
    // 2. in department2, Should disable the option selected in department1
    var check = document.getElementById("dept1").value;
    var i;
    for(i=0;i<4;i++)
      {    
        if(document.getElementById("dept2").options[i].value === check)
        {
  ...
  var constructData = function() {
    ...
    data["pdept"]=document.getElementById("dept1").value;
    data["sdept"]=document.getElementById("dept2").value;
  ...
  var validateResults = function(data) {
    ...
    var mail=data["email"];
    if(reg.exec(mail)==null || name.length>100 || num.length>10)
    isValid=false;
